...
public class DeleteCrea extends AsyncTask<Integer, Void, EncCreaItem> {
    private Context context;
    private AsyncCb<EncCreaItem> callback;
    private Exception exception;
    private EncCreaItem encCreaItem;

    public DeleteCrea (EncCreaItem encCreaItem, Context context, AsyncCb<EncCreaItem> callback){
    ...
    protected EncCreaItem doInBackground(Integer... integers) {
        ...
        try{
            Connections.getInstance(context).getDatabase().getDao().deleteCrea(this.encCreaItem);
        }
        catch(Exception e){
            exception = e;
        }
